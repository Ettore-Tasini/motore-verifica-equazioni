 """
  ...
  def diagnose_step(prev_lhs, prev_rhs, cur_lhs, cur_rhs, var):
      ...
      if None in (pL, pR, cL, cR):
          return "Il passaggio è troppo complesso per essere analizzato con precisione automaticamente: ricontrolla con calma ogni numero e
  segno."
      ...
      for d in sorted(mism, reverse=True)[:2]:
          ...
          if left_unchanged and not right_unchanged:
              parts.append(f"nel {label} a sinistra sei rimasto a {fmt_term(pL[d], d, var)} (invariato), ma a destra sei passato da
  {fmt_term(pR[d], d, var)} a {fmt_term(cR[d], d, var)}: l'operazione va applicata su entrambi i lati")
          elif right_unchanged and not left_unchanged:
              parts.append(f"nel {label} a destra sei rimasto a {fmt_term(pR[d], d, var)} (invariato), ma a sinistra sei passato da
  {fmt_term(pL[d], d, var)} a {fmt_term(cL[d], d, var)}: l'operazione va applicata su entrambi i lati")
          else:
              parts.append(f"nel {label} a sinistra sei passato da {fmt_term(pL[d], d, var)} a {fmt_term(cL[d], d, var)}, a destra invece da
  {fmt_term(pR[d], d, var)} a {fmt_term(cR[d], d, var)}: i due lati devono cambiare allo stesso modo")
      text = ' ; '.join(parts) + '.'
          ...
          if cmp['status'] == 'ok':
              ...
              final = {
                  ...
                  'message': "Non sono riuscito a determinare con certezza la soluzione di questo esercizio "
                             "(potrebbe essere troppo complesso per quest          return ratio_result

      if not row_has_fraction(prev_lhs, prev_rhs, var):
          return {'supported': False}

      D = common_denominator(prev_lhs, prev_rhs, var)
      expected_lhs, expected_rhs = clear_denominator(prev_lhs, prev_rhs,
  D)

      cleared = check_relation(expected_lhs, expected_rhs, cur_lhs,
  cur_rhs, var)
      if not cleared['supported']:
          return {'supported': False}

      result = {'supported': True, 'equivalent': cleared['equivalent'],
  'relation_kind': 'implication'}
      if not cleared['equivalent']:
          result['expected_lhs'] = expected_lhs
          result['expected_rhs'] = expected_rhs
      return result
  ...
  def fmt_term(coeff, degree, var):
      ...
      if coeff == 0:
          return '0'
      sign = '-' if coeff.could_extract_minus_sign() else '+'
      absval = simplify(-coeff if coeff.could_extract_minus_sign() else
  coeff)
      numstr = fmt_num(absval)
      if degree == 0:
          return f"{sign}{numstr}"
      varpart = str(var) if degree == 1 else f"{var}^{degree}"
      return f"{sign}{varpart}" if numstr == '1' else
  f"{sign}{numstr}{varpart}"
  ...
  def side_has_content(pSide, cSide):
      """True if this side is ever anything other than plain 0, before or
  after."""
      return any(simplify(pSide[d]) != 0 for d in (0, 1, 2)) or
  any(simplify(cSide[d]) != 0 for d in (0, 1, 2))
  ...
  def format_expr(expr):
      """Renders a SymPy expression in the same house style used
  elsewhere
      (2x not 2*x, x^2 not x**2) instead of SymPy's own str()
  conventions."""
      return _house_style(str(expand(expr)))


  def format_expr_raw(expr):
      """Same house style as format_expr, but WITHOUT expand() first —
  for
      grounded diagnosis on expressions with a fraction, where expand()
  would
      distribute the numerator over the denominator (e.g. (x+1)/(x-2)
  becomes
      x/(x-2)+1/(x-2)), no longer matching what the student actually
  wrote."""
      return _house_style(str(expr))


  def diagnose_step(prev_lhs, prev_rhs, cur_lhs, cur_rhs, var):
      """Per-side (left vs right) coefficient comparison — same style as
  the
      JS diagnosis, but exact/symbolic instead of numeric-fitted."""
      pL = poly_coeffs(prev_lhs, var); pR = poly_coeffs(prev_rhs, var)
      cL = poly_coeffs(cur_lhs, var); cR = poly_coeffs(cur_rhs, var)
      if None in (pL, pR, cL, cR):
          return "Il passaggio è troppo complesso per essere analizzato
  con precisione automaticamente: ricontrolla con calma ogni numero e
  segno."

      relevant = [d for d in (0, 1, 2) if any(simplify(x[d]) != 0 for x
  in (pL, pR, cL, cR))]
      mism = [d for d in relevant if simplify((cL[d] - pL[d]) - (cR[d] -
  pR[d])) != 0]

      if len(mism) == 0:
          return None  # shouldn't happen if caller already knows it's an
  error
      if len(mism) >= 3 or (len(relevant) > 1 and len(mism) >=
  len(relevant)):
          return "Qui non torna quasi nulla: ricontrolla questo passaggio
  con calma, sembra che più cose insieme non quadrino."

      # Is every mismatch confined to ONE side, while the OTHER side
  never has
      # any content at all (not just "unchanged at this degree", but
  plainly
      # always 0 everywhere)? That's not "you forgot to do it on both
  sides" —
      # there's nothing on the other side to do it to. It's a bad rewrite
      # WITHIN one side (wrong distribution/factoring), and needs a
  completely
      # different message: compare the whole side before/after, not term
  by term.
      right_is_trivial = not side_has_content(pR, cR)
      left_is_trivial = not side_has_content(pL, cL)
      mism_all_on_left = all(simplify(pR[d] - cR[d]) == 0 for d in mism)
      ...
      if mism_all_on_left and right_is_trivial:
          prev_s, cur_s = format_expr(prev_lhs), format_expr(cur_lhs)
          return (f"Hai riscritto il lato sinistro in modo scorretto:
  prima era {prev_s}, ora è {cur_s} — "
                  f"non sono la stessa espressione. Se hai provato a
  raccogliere un fattore comune, "
                  f"controlla che moltiplichi davvero TUTTI i termini
  dentro la parentesi, non solo alcuni.")
      if mism_all_on_right and left_is_trivial:
          prev_s, cur_s = format_expr(prev_rhs), format_expr(cur_rhs)
          return (f"Hai riscritto il lato destro in modo scorretto: prima
  era {prev_s}, ora è {cur_s} — "
                  f"non sono la stessa espressione. Se hai provato a
  raccogliere un fattore comune, "
                  f"controlla che moltiplichi davvero TUTTI i termini
  dentro la parentesi, non solo alcuni.")
      ...
      for d in sorted(mism, reverse=True)[:2]:
          ...
          right_unchanged = simplify(pR[d] - cR[d]) == 0
          if left_unchanged and not right_unchanged:
              parts.append(f"nel {label} a sinistra sei rimasto a
  {fmt_term(pL[d], d, var)} (invariato), ma a destra sei passato da
  {fmt_term(pR[d], d, var)} a {fmt_term(cR[d], d, var)}: l'operazione va
  applicata su entrambi i lati")
          elif right_unchanged and not left_unchanged:
              parts.append(f"nel {label} a destra sei rimasto a
  {fmt_term(pR[d], d, var)} (invariato), ma a sinistra sei passato da
  {fmt_term(pL[d], d, var)} a {fmt_term(cL[d], d, var)}: l'operazione va
  applicata su entrambi i lati")
          else:
              parts.append(f"nel {label} a sinistra sei passato da
  {fmt_term(pL[d], d, var)} a {fmt_term(cL[d], d, var)}, a destra invece
  da {fmt_term(pR[d], d, var)} a {fmt_term(cR[d], d, var)}: i due lati
  devono cambiare allo stesso modo")
      text = ' ; '.join(parts) + '.'
      return text[0].upper() + text[1:]


  FRACTION_IMPLICATION_NOTE = (
      "Hai moltiplicato per un'espressione che contiene la incognita: da
  qui in "
      "poi potrebbero comparire soluzioni che non vanno bene
  nell'equazione di "
      "partenza — verrà controllato alla fine."
  )


  def diagnose_fraction_step(prev_lhs, prev_rhs, expected_lhs,
  expected_rhs, cur_lhs, cur_rhs, var):
      """Diagnosi per un errore nella cancellazione di un denominatore:
  una
      frase-ponte mostra la riga originale con la frazione (nella
  notazione
      dello studente) e cosa ci si aspetta dopo averla cancellata
      correttamente, poi riusa il confronto coefficiente-per-coefficiente
  di
      diagnose_step tra la forma attesa e quella scritta dallo
  studente."""
      prev_s = f"{format_expr_raw(prev_lhs)} =
  {format_expr_raw(prev_rhs)}"
      expected_s = f"{format_expr(expected_lhs)} =
  {format_expr(expected_rhs)}"
      detail = diagnose_step(expected_lhs, expected_rhs, cur_lhs,
  cur_rhs, var)
      bridge = f"Eliminando il denominatore da {prev_s} ci si aspetta
  {expected_s}. "
      return bridge + (detail or "")


  def solve_original(lhs, rhs, var):
      """Solves the ORIGINAL equation (first row) for the true solution
  set,
      used by the final check. Real solutions only, matching the scope of
      these grades (no complex roots expected/taught here)."""
      from sympy import solveset
      result = solveset(Eq(lhs, rhs), var, domain=S.Reals)
      if result == S.EmptySet and simplify(cancel(lhs - rhs)) == 0:
          # equazione razionale che è in realtà un'identità (vera per
  ogni x
          # tranne i poli): solveset può collassare erroneamente a
  EmptySet
          # invece di Reals meno i poli — non fidarsi, dichiarare
  'unknown'.
          return None
      return result
  ...
  def parse_declared_solutions(text, var):
      """Parses a student's final-answer line into a list of SymPy
  values.
      Accepts 'x=3', 'x=2 or x=3', 'x=2, x=3', etc."""
      parts = _SPLIT_RE.split(text)
      values = []
  ...
  _EMPTY_PHRASES = {
      'impossibile', 'nessunasoluzione', 'nessunesoluzioni',
  'nonesistesoluzione',
      'nonesistonosoluzioni', 'insiemevuoto', 's=∅', '∅', 'nessuna',
  'nosolution',
      'impossible', 's={}', 's=vuoto', 'equazioneimpossibile',
  }
  ...
  def is_declared_empty(text):
      """Riconosce risposte come 'impossibile', 'nessuna soluzione', 'S =
  ∅', ecc.
      come dichiarazione esplicita che l'equazione non ha soluzioni — da
  NON
      confondere con un testo che semplicemente non si riesce a
  interpretare."""
      t = text.strip().lower().replace(' ', '').rstrip('.')
      return t in _EMPTY_PHRASES
  ...
  declared_is_empty_claim=False):
      ...
      if declared_is_empty_claim:
          if len(true_vals) == 0:
              return {'status': 'ok', 'correct': []}
          return {'status': 'wrongly_claimed_impossible', 'missing':
  true_vals, 'correct': true_vals}
      ...
      missing = [t for i, t in enumerate(true_vals) if i not in matched]
      if invalid:
          return {'status': 'invalid_solution_present', 'invalid':
  invalid, 'missing': missing, 'correct': true_vals}
      if missing:
          return {'status': 'incomplete', 'missing': missing, 'correct':
  true_vals}
      return {'status': 'ok', 'correct': true_vals}


  # Ruoli che questa prima versione sa gestire per il confronto
  passo-passo.
  # Gli altri (domain, substitution, side_calc, case_split) sono
  riconosciuti
  # dal contratto ma non ancora implementati: lo diciamo onestamente
  invece
  # di ignorarli o fingere di capirli.
  SUPPORTED_STEP_ROLES = {"equation", None, "equazione", ""}
  ...
  def process_sheet(rows, variable_hint=None):
      """Framework-independent request handler: rows is a list of plain
  dicts
      with keys index/plain/role_hint (latex is accepted but unused
  server-side).
      Returns a plain dict matching the steps/final_check contract
  exactly —
      no FastAPI/Pydantic dependency, so this can be unit-tested
  directly."""
      steps = []
      prev_parsed = None
      first_equation = None
      ...
      for row in rows:
          ...
          if role in ('solution', 'soluzione'):
              solution_row = row
              steps.append({'index': idx, 'status': 'skip', 'relation':
  'solution'})
              continue
          ...
          if role not in SUPPORTED_STEP_ROLES:
              steps.append({
                  'index': idx, 'status': 'unreadable', 'relation': role,
                  'note': f"Le righe di tipo '{role}' non sono ancora
  supportate in questa versione: "
                          f"per ora vengono ignorate nel controllo, non
  contano né come errore né come ok."
              })
              continue
          ...
          try:
              ...
              steps.append({'index': idx, 'status': 'unreadable',
                            'note': f"Non sono riuscito a interpretare
  questa riga come equazione: {e}"})
              continue
          ...
          if not rel['supported'] and (row_has_fraction(p_lhs, p_rhs,
  var) or row_has_fraction(lhs, rhs, var)):
              rel = check_fraction_relation(p_lhs, p_rhs, lhs, rhs, var)
              relation_label = rel.get('relation_kind')
          ...
          if not rel['supported']:
              steps.append({
                  'index': idx, 'status': 'unreadable', 'relation': None,
                  'note': "Questo passaggio è troppo complesso per questa
  versione del motore "
                          "(per ora gestisce solo equazioni di 1°/2°
  grado, anche con frazioni algebriche, "
                          "in una sola variabile, senza parametri)."
              })
          elif rel['equivalent']:
              note = FRACTION_IMPLICATION_NOTE if relation_label ==
  'implication' else None
              steps.append({'index': idx, 'status': 'ok', 'relation':
  relation_label, 'note': note})
          else:
              if relation_label == 'implication':
                  diag = diagnose_fraction_step(p_lhs, p_rhs,
  rel['expected_lhs'], rel['expected_rhs'], lhs, rhs, var)
              else:
                  diag = diagnose_step(p_lhs, p_rhs, lhs, rhs, var)
              steps.append({'index': idx, 'status': 'error', 'relation':
  relation_label, 'diagnosis': diag})
      ...
      if first_equation is None or solution_row is None or not
  (solution_row.get('plain') or '').strip():
          final = {'status': 'no_final_solution',
                   'message': 'Non ho trovato una riga di soluzione
  finale da controllare.'}
      else:
          f_lhs, f_rhs, f_var = first_equation
          try:
              true_set = solve_original(f_lhs, f_rhs, f_var)
              if true_set is None:
                  ...
                  plain_text = solution_row['plain']
                  if is_declared_empty(plain_text):
                      cmp = compare_solutions(true_set, [],
  declared_is_empty_claim=True)
                  else:
                      declared = parse_declared_solutions(plain_text,
  f_var)
                      cmp = compare_solutions(true_set, declared)
          except Exception:
              cmp = {'status': 'unknown'}
          ...
          if cmp['status'] == 'ok':
              final = {'status': 'ok', 'message': 'Hai trovato tutte le
  soluzioni corrette.'}
          elif cmp['status'] == 'wrongly_claimed_impossible':
              correct = [str(v) for v in cmp['correct']]
              final = {
                  'status': 'incomplete',
                  'message': "In realtà l'equazione ha soluzione: hai
  detto che era impossibile, ma non lo è. "
                             f"{'La soluzione corretta è' if len(correct)
  == 1 else 'Le soluzioni corrette sono'}: {', '.join(correct)}.",
                  'correct_solutions': correct,
              }
          elif cmp['status'] == 'incomplete':
              missing = [str(v) for v in cmp['missing']]
              final = {
                  'status': 'incomplete',
                  'message': "Le soluzioni che hai scritto sono corrette,
  ma non sono tutte: "
                             f"ti manca {'la soluzione' if len(missing)
  == 1 else 'le soluzioni'} {', '.join(missing)}.",
                  'correct_solutions': [str(v) for v in cmp['correct']],
              }
          elif cmp['status'] == 'invalid_solution_present':
              invalid = [str(v) for v in cmp['invalid']]
              if not cmp['correct']:
                  final = {
                      'status': 'invalid_solution_present',
                      'message': f"{'Il valore' if len(invalid) == 1 else
  'I valori'} {', '.join(invalid)} "
                                 f"che hai scritto non {'è una soluzione
  valida' if len(invalid) == 1 else 'sono soluzioni valide'}: "
                                 "questa equazione in realtà non ha
  soluzioni reali (è impossibile).",
                      'correct_solutions': [],
                  }
              else:
                  final = {
                      'status': 'invalid_solution_present',
                      'message': f"{'La soluzione' if len(invalid) == 1
  else 'Le soluzioni'} {', '.join(invalid)} "
                                 f"non {'è valida' if len(invalid) == 1
  else 'sono valide'} per l'equazione di partenza. "
                                 f"Le soluzioni corrette sono: {',
  '.join(str(v) for v in cmp['correct'])}.",
                      'correct_solutions': [str(v) for v in
  cmp['correct']],
                  }
          else:
              final = {
                  'status': 'unknown',
                  'message': "Non sono riuscito a determinare con
                                 f"non {'è valida' if len(invalid) == 1
  else 'sono valide'} per l'equazione di partenza. "
                                 f"Le soluzioni corrette sono: {',
  '.join(str(v) for v in cmp['correct'])}.",
  ...
  cmp['correct']],
              ...
              final = {
                  'status': 'unknown',
                  'message': "Non sono riuscito a determinare con
  certezza la soluzione di questo esercizio "
                             "(potrebbe essere troppo complesso per
  questa versione del motore)."
              }
